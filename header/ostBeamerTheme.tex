...
\newtcolorbox{OSTBox}[1]{colback=white,colframe=OSTPink,title=#1}
...
\setbeamertemplate{footline}{
    ...
    \begin{tikzpicture}[overlay, remember picture,shift=(current page.south west)]
        ...
        \fill[fill=OSTPurple] (1.0525cm, 0) rectangle (1.0535cm, 0.528925);
    \end{tikzpicture}
}

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Color scheme and font setup
\setbeamercolor*{frametitle}{fg=OSTPurple,bg=white}
\setbeamerfont{frametitle}{series=\bfseries}

\setbeamertemplate{section in toc}{\hspace{1em}{\scriptsize\color{OSTPurple}$\bullet$}~\inserttocsection}
...
\setbeamercolor*{itemize item}{fg=OSTPurple}
...
\setbeamercolor{caption name}{fg=OSTPurple}
...
\defbeamertemplate*{title page}{customized}[1][]{
    ...
    \begin{tikzpicture}[remember picture,overlay,shift=(current page.south west)]
        ...
        \ifthenelse{\lengthtest{\beamerinternalpaperwidth=16cm}}{%
            ...
            % tITLE
            \node[anchor=north west, draw=none, opacity=0.8, fill=white, minimum height=1.048406cm, inner sep=0.1511cm] at ($(current page.north west)+(7.0035419, -4.4958678)$) {\parbox{7.81582cm}{\color{OSTPurple}\Large\bfseries\Title}};
            
            % Subtitle
            \node[anchor=north west, draw=none, opacity=0.8, fill=OSTPink, minimum height=0.6422668cm, inner sep=0.1511cm] at ($(current page.north west)+(5.5442739, -5.5442739)$) {\parbox{5.331759cm}{\color{white}\Subtitle}};
            ...
            % Title
            \node[anchor=north west, draw=none, opacity=0.8, fill=white, minimum height=0.942362cm, inner sep=0.1511cm] at ($(current page.north west)+(3.91055, -4.96882)$) {\parbox{7.942047cm}{\color{OSTPurple}\Large\bfseries\Title}};

            % Subtitle
            \node[anchor=north west, draw=none, opacity=0.8, fill=OSTPink, minimum height=0.68535cm, inner sep=0.1511cm] at ($(current page.north west)+(2.31811, -5.91118)$) {\parbox{5.99685cm}{\color{white}\Subtitle}};
